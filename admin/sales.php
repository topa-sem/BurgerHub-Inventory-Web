...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['process_sale'])) {
    ...
    try {
        ...
        foreach ($_SESSION['cart'] as $product_id => $quantity_sold) {
            ...
            // 3. Check user's stock for each ingredient
            while ($ingredient = $recipe_result->fetch_assoc()) {
                $ingredient_id = $ingredient['ingredient_id'];
                $total_required = $ingredient['quantity_required'] * $quantity_sold;

                // Check user's stock level
                $stock_stmt = $conn->prepare("
                    SELECT uis.current_stock_quantity, i.name 
                    FROM USER_INGREDIENT_STOCK uis 
                    JOIN INGREDIENT i ON uis.ingredient_id = i.ingredient_id 
                    WHERE uis.ingredient_id = ? AND uis.user_id = ?
                ");
                $stock_stmt->bind_param("ii", $ingredient_id, $user_id);
                $stock_stmt->execute();
                $stock_data = $stock_stmt->get_result()->fetch_assoc();
                $stock_stmt->close();

                // If no stock record exists or insufficient stock
                if (!$stock_data || $stock_data['current_stock_quantity'] < $total_required) {
                    $insufficient_stock = true;
                    $available = $stock_data ? $stock_data['current_stock_quantity'] : 0;
                    $error_details = "Not enough stock for ingredient: " . htmlspecialchars($stock_data['name'] ?? 'Unknown') . 
                                     ". (Required: $total_required, Available: $available)";
        ...
        foreach ($cart_items_details as $product_id => $item) {
            ...
            // 2b. Deduct ingredients from user's stock based on recipe
            $recipe_stmt = $conn->prepare("SELECT ingredient_id, quantity_required FROM RECIPE WHERE product_id = ?");
            $recipe_stmt->bind_param("i", $product_id);
            $recipe_stmt->execute();
            $recipe_result = $recipe_stmt->get_result();
            ...
            while ($ingredient = $recipe_result->fetch_assoc()) {
                ...
                ");
                $deduct_stmt->bind_param("dii", $total_to_deduct, $ingredient['ingredient_id'], $user_id);
                $deduct_stmt->execute();
                $deduct_stmt->close();
            }
            $recipe_stmt->close();
...
$sales_stmt = $conn->prepare("
    SELECT s.sale_id, s.total_revenue, TIME(s.sale_date) AS sale_time, u.username 
    FROM SALE s 
...
<div class="container">
    ...
    <div class="main">
        ...
        <!-- Bottom Section: Today's Sales -->
        <div class="panel">
            <h2>Sales for <?php echo date('d M Y', strtotime($selected_date)); ?></h2>
            <table>
                <thead>
                    <tr>
                        <th>Sale ID</th>
                ...
                <tbody>
                    ...
                    $sales_result = $sales_stmt->get_result();
                    while ($row = $sales_result->fetch_assoc()) {
                        $selected_date_sales[] = $row;
                    }
                    $sales_stmt->close();
                    
                    if (empty($selected_date_sales)): ?>
                        <tr><td colspan="4">No sales recorded for this date.</td></tr>
                    <?php else: ?>
                        <?php foreach ($selected_date_sales as $sale): ?>
                        <tr>
