...
if (isset($_GET['manage_order_id'])) {
    ...
    // Verify the order belongs to the current manager
    $stmt = $conn->prepare("SELECT user_id FROM `ORDER` WHERE order_id = ?");
    $stmt->bind_param("i", $manage_order_id);
    $stmt->execute();
    $order_owner = $stmt->get_result()->fetch_assoc()['user_id'] ?? null;
    $stmt->close();

    if ($order_owner !== $user_id) {
        header("Location: orders.php?error=Access+denied");
        exit();
    }
...
if ($manage_order_id) {
    ...
    // 2. Pending orders (only current manager's orders)
    $stmt = $conn->prepare("
        SELECT O.*, S.name AS supplier_name
        FROM `ORDER` O
        JOIN SUPPLIER S ON O.supplier_id = S.supplier_id
    ...
    // 3. Past orders (only current manager's orders)
    ...
    $stmt = $conn->prepare("
        ...
        AND DATE(O.order_date) = ? AND O.user_id = ?
        ORDER BY O.order_date DESC
        LIMIT 10
    ");
...
<div class="container">
    ...
    <div class="main">
        ...
        <?php if ($manage_order_id): ?>
            <h2>Manage My Order #<?php echo htmlspecialchars($manage_order['order_id']); ?></h2>
            <div class="order-header">
                ...
                <a href="orders.php" class="back-link">&larr; Back to my orders</a>
            </div>
        ...
        <?php else: ?>
                ...
                <table>
                    ...
                    <tbody>
                        <?php if (empty($pending_orders)): ?>
                            <tr><td colspan="6">You have no pending orders.</td></tr>
                        <?php else: ?>
            ...
            <h2>My Order History for <?php echo date('d M Y', strtotime($selected_date)); ?></h2>
            <div class="panel">
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <?php if (empty($past_orders)): ?>
                            <tr><td colspan="5">No order history for this date.</td></tr>
                        <?php else: ?>
