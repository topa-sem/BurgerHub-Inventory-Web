...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($email) || empty($password) || empty($re_password) || empty($user_type)) {
    ...
    $insert_sql = "INSERT INTO USER (username, email, password_hash, branch, account_balance, user_type)
               VALUES (?, ?, ?, ?, 5000.00, ?)";
    $stmt = $conn->prepare($insert_sql);
    $stmt->bind_param("sssss", $username, $email, $password_hash, $branch, $user_type);
